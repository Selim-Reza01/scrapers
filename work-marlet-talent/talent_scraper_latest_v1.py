...
# ========== NEW FEATURES ==========
def is_valid_for_batch(rec: dict) -> bool:
    has_first_name = bool((rec.get("first_name") or "").strip())
    has_location = bool((rec.get("location") or "").strip())
    has_contact = bool(
        (rec.get("email") or "").strip() or 
        (rec.get("work_phone") or "").strip() or 
        (rec.get("mobile_phone") or "").strip()
    )
    return has_first_name and has_location and has_contact

def generate_filename(filter_chips: list, total_count: int) -> Path:
    timestamp = datetime.now().strftime("%Y-%m-%d")
    
    if filter_chips:
        filter_text = "_".join(filter_chips)
        filter_text = re.sub(r'[<>:"/\\|?*]', '', filter_text)
        filter_text = re.sub(r'[\s:,;]+', '_', filter_text)
        if len(filter_text) > 100:
            filter_text = filter_text[:100]
        base = f"{filter_text}_{total_count}_{timestamp}"
    else:
        base = f"no_filter_{total_count}_{timestamp}"
    ...
    while path.exists():
        if filter_chips:
            ...
            if len(filter_text) > 100:
                filter_text = filter_text[:100]
            path = OUTDIR / f"{filter_text}_{total_count}_{timestamp} ({i}).xlsx"
        else:
            path = OUTDIR / f"no_filter_{total_count}_{timestamp} ({i}).xlsx"
        i += 1
    return path

def save_batch_to_excel(rows: list, filepath: Path, filter_chips: list, append_mode=False):
    import pandas as pd
    ...
    if append_mode and filepath.exists():
        try:
            with pd.ExcelWriter(filepath, engine="openpyxl", mode="a", if_sheet_exists="overlay") as writer:
                existing_df = pd.read_excel(filepath, sheet_name="Sheet1", skiprows=1)
                start_row = len(existing_df) + 2
                df.to_excel(writer, index=False, header=False, startrow=start_row, sheet_name="Sheet1")
        except Exception:
            with pd.ExcelWriter(filepath, engine="xlsxwriter") as writer:
                ...
                ws.merge_range(0, 1, 0, len(COLS)-1, " ; ".join(filter_chips))
            start_row = 2
    else:
        with pd.ExcelWriter(filepath, engine="xlsxwriter") as writer:
            filt_df = pd.DataFrame([{k:"" for k in COLS}], columns=COLS)
...
def run_sync():
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            while batch_size <= 0:
                try:
                    batch_input = input("\nEnter batch size (number of valid records per batch save): ").strip()
            ...
            choice = input("\nWant to filter Talent? press 1 to filter, or press 0 to continue without filtering: ").strip()
            if choice == "1":
            ...
            print(f"\n--- Scraping from page {start_page} to {end_page} with batch size {batch_size} ---")  # NEW
            ...
            def process_payload(payload, page_index):
                nonlocal valid_batch_records, excluded_records, batch_count, total_saved, output_file, filter_chips  # NEW
                ...
                for idx, row in enumerate(payload, start=1):
                    ...
                    if key not in seen_ids:
                        ...
                        # Batch logic
                        if is_valid_for_batch(rec):
                            valid_batch_records.append(rec)
                            batch_count += 1
                            
                            if batch_count >= batch_size:
                                # Get filter chips once
                                if not filter_chips:
                                    try:
                                        filter_chips = page.evaluate(FILTER_CHIPS_JS) or []
                                    except Exception:
                                        filter_chips = []
            
                                if output_file is None:
                                    output_file = generate_filename(filter_chips, 0)
                                    row_start = save_batch_to_excel(valid_batch_records, output_file, filter_chips, append_mode=False)
                                    total_saved += len(valid_batch_records)
                                    print(f"\n\nBatch {batch_size} reached! Saved {len(valid_batch_records)} records at row {row_start}")
                                else:
                                    row_start = save_batch_to_excel(valid_batch_records, output_file, filter_chips, append_mode=True)
                                    total_saved += len(valid_batch_records)
                                    print(f"\n\nBatch {batch_size} reached! Extended file with {len(valid_batch_records)} records at row {row_start}")
            ...
            print(f"\n\n--- Scraping Complete ---")
            print(f"Total Talent Scraped: {len(rows_out)}")
            print(f"Total Valid Records: {total_saved + len(valid_batch_records)}")
            print(f"Total Excluded Records: {len(excluded_records)}")
            
            # Get filter chips if not already fetched
            if not filter_chips:
                try:
                    filter_chips = page.evaluate(FILTER_CHIPS_JS) or []
                except Exception:
            ...
            if valid_batch_records:
                if output_file is None:
                    output_file = generate_filename(filter_chips, len(valid_batch_records) + len(excluded_records))
                    row_start = save_batch_to_excel(valid_batch_records, output_file, filter_chips, append_mode=False)
                    print(f"\nSaved remaining {len(valid_batch_records)} valid records at row {row_start}")
                else:
                    row_start = save_batch_to_excel(valid_batch_records, output_file, filter_chips, append_mode=True)
                    print(f"\nExtended file with remaining {len(valid_batch_records)} valid records at row {row_start}")
                total_saved += len(valid_batch_records)
            
            if excluded_records:
                if output_file is None:
                    output_file = generate_filename(filter_chips, len(excluded_records))
                    row_start = save_batch_to_excel(excluded_records, output_file, filter_chips, append_mode=False)
                    print(f"\nSaved {len(excluded_records)} excluded records at row {row_start}")
                else:
                    row_start = save_batch_to_excel(excluded_records, output_file, filter_chips, append_mode=True)
                    print(f"\nExtended file with {len(excluded_records)} excluded records at row {row_start}")
            
            if output_file:
                final_count = total_saved + len(excluded_records)
                final_file = generate_filename(filter_chips, final_count)
            ...
            print("1. Run again (with new filter)")
